...
class MIRTDDPGTrainer:
    def __init__(self, config, device=None):
        ...
        self.max_steps=int(t.get('selection_horizon', t.get('max_steps',20))); self.selection_horizon=self.max_steps; self.warm_start_items=1; self.epochs=int(t.get('epochs',1)); self.batch_size=int(t.get('batch_size',32))
        ...
        self.theta_cfg=canonical_theta_fit(config.get('theta_fit',{}), smoke=bool(t.get('smoke')))
    ...
    def _split(self, seqs):
        ...
        for seq in seqs:
            ...
            manifest['students'][seq.student_id]={'split':'train' if seq.student_id in train else 'validation','warm_start_item':support[0],'warm_start_response':seen[support[0]],'candidate_items':support[1:],'query_items':query,'support':support,'query':query}
    ...
    def _validate(self, actor, rows, normalizer):
        ...
        for sid,is_train,supp,sr,query,qr in rows:
            ...
            hist_i=[supp[0]]; hist_r=[sr[0]]; cand=list(supp[1:])
    ...
    def train(self):
        ...
        normalizer=compute_action_normalizer(self.mirt, range(self.mirt.n_items), self.device)
        ...
        for ep in range(1,self.epochs+1):
            rewards=[]; metric_acc={'critic_loss':[],'actor_loss':[],'mean_q':[],'target_q_mean':[],'q_clip_fraction':[],'sampled_valid_timesteps':[]}; uniq=set(); updates=0
            for sid,is_train,supp,sr,query,qr in rows:
                if not is_train: continue
                hi=[supp[0]]; hr=[sr[0]]; cand=list(supp[1:]); prev=None
                for step in range(min(self.max_steps,len(cand))):
                    ...
                    if len(rb)>=self.batch_size:
                        ...
                        q=critic(s,a); loss=F.huber_loss(q,y.clamp(-self.q_clip,self.q_clip)); co.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(critic.parameters(),1.0); co.step(); metric_acc['critic_loss'].append(loss.detach().item()); metric_acc['mean_q'].append(q.detach().mean().item()); metric_acc['target_q_mean'].append(y.detach().mean().item()); metric_acc['q_clip_fraction'].append((q.detach().abs()>self.q_clip*.99).float().mean().item()); metric_acc['sampled_valid_timesteps'].append(int(len(rw)))
                        if updates%self.policy_delay==0:
                            pa=actor(s); lossa=-critic(s,pa).mean(); ao.zero_grad(); lossa.backward(); torch.nn.utils.clip_grad_norm_(actor.parameters(),1.0); ao.step(); metric_acc['actor_loss'].append(lossa.detach().item())
                            self._soft(actor,targ_a); self._soft(critic,targ_c)
                        updates+=1; self._assert_frozen()
            vm=self._validate(actor,rows,normalizer); rec={'epoch':ep,'mean_reward':float(np.mean(rewards)) if rewards else 0.0,'selected_unique_items':len(uniq),**{k:(float(np.mean(v)) if v and k!='sampled_valid_timesteps' else int(np.sum(v)) if v else float('nan')) for k,v in metric_acc.items()},**vm}; hist.append(rec)
            qfrac=rec.get('q_clip_fraction', float('nan'))
    ...
    def _save(self,actor,norm,epoch,metrics,name):
        ...
        torch.save({'actor_state_dict':actor.state_dict(),'action_mean':norm.mean,'action_std':norm.std,'mirt_dim':self.mirt.n_dims,'hidden_dim':actor.hidden_dim,'state_definition_version':STATE_DEFINITION_VERSION,'actor_architecture':ACTOR_ARCHITECTURE,'theta_fit':self.theta_cfg,'selection_horizon':self.selection_horizon,'warm_start_items':self.warm_start_items,'action_normalizer_scope':'full_mirt_item_bank','normalizer_item_count':self.mirt.n_items,'training_config':self.config,'epoch':epoch,'validation_metrics':metrics,'git_commit':git}, self.out/name)

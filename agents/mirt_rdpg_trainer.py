...
from evaluation.protocol import canonical_theta_fit
...
class MIRTRDPGTrainer:
    def __init__(self, config, device=None):
        ...
        self.max_steps=int(t.get('selection_horizon', t.get('max_steps',10))); self.selection_horizon=self.max_steps; self.warm_start_items=1; self.epochs=int(t.get('epochs',2)); self.gamma=float(t.get('gamma',.99)); self.tau=float(t.get('tau',.005)); self.policy_delay=int(t.get('policy_delay',2)); self.grad_clip=float(t.get('gradient_clip',1.0)); self.q_clip=float(t.get('q_clip',20.0))
        ...
        self.theta_cfg=canonical_theta_fit(self.config.get('theta_fit',{}), smoke=bool(t.get('smoke')))
    ...
    def _split(self,seqs):
        ...
        for seq in seqs:
            ...
            query=items[:qn]; supp=items[qn:]; manifest['students'][seq.student_id]={'split':'train' if seq.student_id in train else 'validation','warm_start_item':supp[0],'warm_start_response':seen[supp[0]],'candidate_items':supp[1:],'query_items':query,'support':supp,'query':query}; rows.append((seq.student_id,seq.student_id in train,supp,[seen[i] for i in supp],query,[seen[i] for i in query]))
    ...
    def _optimize(self, rb, actor, critic, ta, tc, ao, co, stepno):
        ...
        if um.sum().item() == 0:
            raise ValueError('RDPG optimization batch has zero valid unroll timesteps')
        with torch.no_grad():
        ...
        return {'critic_loss':float(cl.detach()), 'actor_loss':float(al.detach()), 'mean_q':float((q.detach()*um).sum()/um.sum()), 'target_q_mean':float((y.detach()*um).sum()/um.sum()), 'q_clip_fraction':float(((q.detach().abs()>self.q_clip*.99).float()*um).sum()/um.sum()), 'sampled_valid_timesteps':int(um.sum().item()), 'shapes':{k:tuple(v.shape) for k,v in b.items()}}
    def _validate(self,actor,rows,norm):
        vals=[]; auc=[]
        for _,is_train,supp,sr,query,qr in rows:
            if is_train: continue
            hi=[supp[0]]; hr=[sr[0]]; cand=list(supp[1:]); h=actor.init_hidden(1,self.device)
    ...
    def train(self):
        start=time.time(); rows=self._split(self._load_sequences(self.config['assets'].get('train_valid_sequences','kc_level/train_valid_sequences.csv'))); norm=compute_action_normalizer(self.mirt, range(self.mirt.n_items), self.device)
        ...
        for ep in range(1,self.epochs+1):
            rewards=[]; uniq=set(); epoch_metrics={'critic_loss':[],'actor_loss':[],'mean_q':[],'target_q_mean':[],'q_clip_fraction':[],'sampled_valid_timesteps':[]}
            for _,is_train,supp,sr,query,qr in rows:
                if not is_train: continue
                hi=[supp[0]]; hr=[sr[0]]; cand=list(supp[1:]); h=actor.init_hidden(1,self.device); S=[];A=[];R=[];NS=[];D=[]; prev=None
                for step in range(min(self.max_steps,len(cand))):
                ...
                if len(rb)>=self.min_eps:
                    for _ in range(self.updates_per_episode):
                        last=self._optimize(rb,actor,critic,ta,tc,ao,co,optn); optn+=1; self._assert_frozen()
                        for k in epoch_metrics:
                            if k in last and not (k == 'actor_loss' and last[k] != last[k]): epoch_metrics[k].append(last[k])
            vm=self._validate(actor,rows,norm); agg={k:(float(np.mean(v)) if v and k!='sampled_valid_timesteps' else int(np.sum(v)) if v else float('nan')) for k,v in epoch_metrics.items()}; rec={'epoch':ep,'mean_reward':float(np.mean(rewards)) if rewards else 0.0,'selected_unique_items':len(uniq),'replay_episode_count':len(rb),**agg,**vm}; hist.append(rec)

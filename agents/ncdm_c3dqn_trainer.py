...
def forward_q_network(network: nn.Module, batch: dict[str, torch.Tensor]):
...
def compute_double_dqn_loss(
    ...
    dones: torch.Tensor,
    gamma: float,
    non_terminal_indices: Sequence[int] | torch.Tensor | None = None,
    ...
    with torch.no_grad():
        ...
        if next_batch is not None:
            if non_terminal_indices is None:
                idx_t = torch.nonzero(~dones.bool(), as_tuple=False).flatten()
            else:
                idx_t = torch.as_tensor(non_terminal_indices, dtype=torch.long, device=rewards.device)
            if idx_t.numel() > 0:
                ...
                next_q[idx_t] = evaluated
        target = rewards.float() + float(gamma) * next_q
    loss = F.smooth_l1_loss(chosen_q, target)
    return loss, {"mean_q": float(chosen_q.detach().mean().item()), "target_q_mean": float(target.detach().mean().item()), "next_q_mean": float(next_q.detach().mean().item()), "next_action_mean": next_action_mean, "terminal_next_q_max": float(next_q[dones.bool()].abs().max().item()) if dones.bool().any() else 0.0}

def transitions_to_batches(transitions: Sequence[C3DQNTransition], cache: NCDMItemFeatureCache, selection_horizon: int) -> tuple[dict[str, torch.Tensor], dict[str, torch.Tensor] | None, torch.Tensor, torch.Tensor, torch.Tensor | None]:
    ...
    idx_t = torch.tensor(nt_indices, dtype=torch.long, device=cache.device) if nt_indices else None
    return batch, next_batch, rewards, dones, idx_t
...
def build_set_checkpoint_metadata(**kwargs: Any) -> dict[str, Any]:
    ...
    for key in ("candidate_set_encoder", "num_set_layers", "num_inducing_points", "set_attention_heads", "use_relative_features", "set_pool_in_value_head", "full_attention_max_candidates", "debug_mode"):
        if key in cfg:
            meta[key] = cfg[key]
    meta.setdefault("relative_feature_names", ["mastery", "weakness", "difficulty_gap", "coverage_count"])
    meta.setdefault("relative_feature_dim", int(meta["knowledge_dim"]) * 3)
    return meta
...
def load_set_c3dqn_checkpoint(checkpoint_path: str | Path, *, ncdm, q_matrix: torch.Tensor, device: str | torch.device = "cpu", expected_protocol_config: dict[str, Any] | None = None) -> tuple[SetConditionedNCDMQNetwork, dict[str, Any]]:
    ...
    if meta.get("actor_architecture") != "set_conditioned_candidate_attention_dueling_double_dqn":
        raise ValueError(f"not a Set-C3DQN checkpoint: {meta.get('actor_architecture')!r}")
    expected = dict(expected_protocol_config or {})
    expected.setdefault("q_matrix_item_count", int(q_matrix.shape[0])); expected.setdefault("ncdm_item_count", int(ncdm.k_difficulty.num_embeddings)); expected.setdefault("knowledge_dim", int(q_matrix.shape[1]))
    validate_c3dqn_checkpoint_metadata(meta, expected)
    cfg = dict(meta.get("model_config") or {})
    net = SetConditionedNCDMQNetwork(int(meta["knowledge_dim"]), d_model=int(cfg.get("d_model",64)), n_heads=int(cfg.get("n_heads",4)), num_history_layers=int(cfg.get("num_history_layers",1)), dropout=float(cfg.get("dropout",0.0)), candidate_set_encoder=cfg.get("candidate_set_encoder","isab"), num_set_layers=int(cfg.get("num_set_layers",1)), num_inducing_points=int(cfg.get("num_inducing_points",8)), set_attention_heads=int(cfg.get("set_attention_heads", cfg.get("n_heads",4))), use_relative_features=bool(cfg.get("use_relative_features", True)), set_pool_in_value_head=bool(cfg.get("set_pool_in_value_head", True)), full_attention_max_candidates=int(cfg.get("full_attention_max_candidates",128)), debug_mode=bool(cfg.get("debug_mode",False))).to(device)
...
def load_c3dqn_checkpoint(checkpoint_path: str | Path, *, ncdm, q_matrix: torch.Tensor, device: str | torch.device = "cpu", expected_protocol_config: dict[str, Any] | None = None) -> tuple[CandidateConditionedNCDMQNetwork, dict[str, Any]]:
    ...
    if meta.get("actor_architecture") != "candidate_conditioned_attention_dueling_double_dqn":
        raise ValueError(f"not a Base C3DQN checkpoint: {meta.get('actor_architecture')!r}")
    validate_c3dqn_checkpoint_metadata(meta, expected)
    cfg = dict(meta.get("model_config") or {})
...
class NCDMC3DQNTrainer:
    def __init__(self, online_net: CandidateConditionedNCDMQNetwork, target_net: CandidateConditionedNCDMQNetwork, cache: NCDMItemFeatureCache, selection_horizon: int, out_dir: str | Path, gamma: float = 0.99, lr: float = 1e-3, gradient_clip: float = 5.0, **kwargs: Any) -> None:
        ...
        self.candidate_pool_config = dict(kwargs.get("candidate_pool_config") or {"prefilter_enabled": False, "prefilter_top_k": 256})
    ...
    def _fit_alpha(self, hist_i: Sequence[int], hist_r: Sequence[float], initial_alpha=None) -> torch.Tensor:
        ...
        cfg = dict(self.alpha_fit)
        steps = int(cfg.get("initial_steps", cfg.get("steps", 8))) if initial_alpha is None else int(cfg.get("incremental_steps", cfg.get("steps", 3)))
        alpha = fit_student_alpha(self.ncdm, self.q_matrix, hist_i, hist_r, initial_alpha=initial_alpha, steps=steps, lr=float(cfg.get("lr", 0.05)), early_stop_tol=float(cfg.get("early_stop_tol", 1e-5)), grad_clip=cfg.get("grad_clip"), device=self.cache.device)
    ...
    def _prefilter_candidates(self, cand, alpha, mastery, coverage):
        return self.prefilter.select(cand, alpha=alpha, mastery=mastery, coverage_count=coverage)

    def _select(self, hist_i, hist_r, cand, mastery, coverage, policy_step, epsilon=None, alpha=None) -> int:
        filtered = self._prefilter_candidates(cand, alpha, mastery, coverage)
        if not filtered:
            raise ValueError("C3DQN selection requires at least one prefiltered candidate")
        if random.random() < (self._epsilon() if epsilon is None else epsilon):
            return int(random.choice(filtered))
        t0 = time.perf_counter()
        ...
        return int(filtered[int(q.argmax(dim=1).item())])
    ...
    def update_once(self) -> dict[str, float] | None:
        ...
        t0 = time.perf_counter()
        batch, next_batch, rewards, dones, nt_idx = transitions_to_batches(transitions, self.cache, self.selection_horizon)
        self.optim.zero_grad(set_to_none=True)
        with torch.autocast(device_type="cuda", enabled=self.use_amp):
            loss, stats = compute_double_dqn_loss(self.online_net, self.target_net, batch, next_batch, rewards.float(), dones, self.gamma, nt_idx)
    ...
    def _run_episode(self, sp) -> tuple[list[C3DQNTransition], list[dict[str, float]], list[int], list[float]]:
        ...
        for t in range(min(self.selection_horizon, len(cand))):
            ...
            item = self._select(hist_i, hist_r, cand, mastery, coverage_count, t, alpha=alpha)
            before_i, before_r, before_c, before_m, before_cov = list(hist_i), list(hist_r), self._prefilter_candidates(cand, alpha, mastery, coverage_count), mastery.clone(), coverage_count.clone()
            cand.remove(item)
            hist_i.append(item)
            hist_r.append(resp[item])
            alpha2 = self._fit_alpha(hist_i, hist_r, initial_alpha=alpha)
            ...
            tr = C3DQNTransition(before_i, before_r, before_c, before_m.detach().cpu().tolist(), (before_cov / max(1, self.selection_horizon)).clamp(0, 1).detach().cpu().tolist(), t, item, r.total, comps, list(hist_i), list(hist_r), self._prefilter_candidates(cand, alpha2, mastery2, coverage2), mastery2.detach().cpu().tolist(), (coverage2 / max(1, self.selection_horizon)).clamp(0, 1).detach().cpu().tolist(), t + 1, (not cand) or t + 1 >= self.selection_horizon)
            self.replay.push(tr)
    ...
    def validate(self, seqs: Sequence[StudentSequence], *, seed: int, query_ratio: float, min_query_items: int) -> dict[str, float]:
        ...
        for seq in seqs:
            ...
            for t in range(min(self.selection_horizon, len(cand))):
                ...
                item = self._select(hist_i, hist_r, cand, mastery, cov, t, epsilon=0.0, alpha=alpha)
                cand.remove(item)
                hist_i.append(item)
                hist_r.append(resp[item])
                alpha = self._fit_alpha(hist_i, hist_r, initial_alpha=alpha)

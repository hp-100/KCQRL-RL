...
class DDPGMIRTPolicy(BaseCATPolicy):
    ...
    def __init__(self, checkpoint, mirt, theta_cfg=None, device='cpu'):
        ...
        if ck.get('action_normalizer_scope') != 'full_mirt_item_bank': raise ValueError(f'{self.name} checkpoint action_normalizer_scope must be full_mirt_item_bank, got {ck.get("action_normalizer_scope")!r}')
        if 'normalizer_item_count' not in ck: raise KeyError(f'{self.name} checkpoint missing normalizer_item_count')
        if int(ck.get('normalizer_item_count')) != int(mirt.n_items): raise ValueError(f'{self.name} checkpoint normalizer_item_count mismatch: checkpoint={int(ck.get("normalizer_item_count"))}, mirt={int(mirt.n_items)}')

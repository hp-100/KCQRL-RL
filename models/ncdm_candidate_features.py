...
def build_global_feature(mastery: torch.Tensor, coverage: torch.Tensor, policy_step: int, selection_horizon: int) -> torch.Tensor:
    ...
    coverage = (coverage.float().flatten() / float(selection_horizon)).clamp(0, 1)
...
def pad_c3dqn_batch(samples: Sequence[dict], cache: NCDMItemFeatureCache, selection_horizon: int) -> dict[str, torch.Tensor]:
    ...
    for row, s in enumerate(samples):
        ...
        cov_count = torch.as_tensor(s.get("coverage_count", s["coverage"]), dtype=torch.float32, device=cache.device)
        coverage_count[row] = cov_count
        glob[row] = build_global_feature(torch.as_tensor(s["mastery"], device=cache.device), cov_count, int(s["policy_step"]), selection_horizon)

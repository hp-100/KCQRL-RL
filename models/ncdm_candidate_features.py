...
def build_global_feature(mastery: torch.Tensor, coverage: torch.Tensor, policy_step: int, selection_horizon: int) -> torch.Tensor:
    ...
    coverage = (coverage.float().flatten() / float(selection_horizon)).clamp(0, 1) if coverage.float().flatten().max().item() > 1.0 else coverage.float().flatten().clamp(0, 1)
...
def pad_c3dqn_batch(samples: Sequence[dict], cache: NCDMItemFeatureCache, selection_horizon: int) -> dict[str, torch.Tensor]:
    ...
    cov_count = torch.zeros((b, k), device=cache.device)
    ...
    for row, s in enumerate(samples):
        ...
        raw_cov = torch.as_tensor(s.get("coverage_count", s["coverage"]), device=cache.device).float()
        cov_count[row] = raw_cov
        glob[row] = build_global_feature(torch.as_tensor(s["mastery"], device=cache.device), raw_cov, int(s["policy_step"]), selection_horizon)
        action[row] = cids.index(int(s["selected_item_id"]))
    return {"history_features": hist, "history_mask": hist_mask, "candidate_features": cand, "candidate_mask": cand_mask, "global_features": glob, "coverage_count": cov_count, "action_index": action}

...
def pad_c3dqn_batch(samples: Sequence[dict], cache: NCDMItemFeatureCache, selection_horizon: int) -> dict[str, torch.Tensor]:
    ...
    for row, s in enumerate(samples):
        ...
        cov_tensor = torch.as_tensor(s["coverage"], device=cache.device).float()
        glob[row] = build_global_feature(torch.as_tensor(s["mastery"], device=cache.device), cov_tensor, int(s["policy_step"]), selection_horizon)
        coverage_count[row] = cov_tensor * float(selection_horizon) if cov_tensor.max().item() <= 1.0 else cov_tensor
